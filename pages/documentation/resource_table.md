...
- **registry**: 4 registries are supported: [Bio.tools](https://bio.tools), [FAIRsharing.org](https://fairsharing.org/), [TeSS](https://tess.elixir-europe.org/) and [EuroPMC](https://europepmc.org/). The keywords you can use respectively are: `biotools`, `fairsharing`, `tess` and `europmc`, specifying the id or query with a colon. FAIRsharing records should use the DOI suffix, without the `FAIRsharing.` prefix. List registries under the `registry` attribute as `key: value pairs`.
